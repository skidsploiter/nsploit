...
namespace XenoUI
{
    ...
    {
        public string XenoVersion = "1.0.9";
        ...
        private async void LoadSupportedVersion()
        {
            try
            {
                ...
                {
                    MessageBox.Show($"The current version {XenoVersion} is outdated.\n\nPlease download the latest version of Niggasploit ({latestVersion}) here: https://discord.gg/qZb6U839", "Outdated Niggasploit version, please DM the developer", MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        private static int GetClientId(string content) => int.Parse(content.Split(new[] { ", PID: " }, StringSplitOptions.None)[1]);
        private static string GetClientName(string content) => content.Split(new[] { ", PID: " }, StringSplitOptions.None)[0].Trim();
